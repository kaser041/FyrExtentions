...
const pkg_2 = require("./pkg");
const crypto_1 = require("crypto");
class CodeGenerator {
    constructor(tc, backend, disableNullCheck) {
        this.funcs = new Map();
        this.globalVars = new Map();
        this.typeCodeMap = new Map();
        this.destructors = new Map();
        this.structs = new Map();
        this.ifaceDescriptors = new Map();
    ...
    processModule(mnode, emitIR, initPackages, duplicateCodePackages) {
        ...
        for (let fnode of mnode.statements) {
            for (let name of fnode.scope.elements.keys()) {
                ...
                if (e instanceof typecheck_1.Function && e.isImported) {
        ...
        for (let name of scope.elements.keys()) {
            ...
            if (e instanceof typecheck_1.Function) {
                if (e.isImported) {
                    throw "Implementation error";
                }
                let name = e.name;
                if (e.type.objectType) {
                    let t = typecheck_1.RestrictedType.strip(e.type.objectType);
                    if (t instanceof typecheck_1.PointerType) {
                        t = typecheck_1.RestrictedType.strip(t.elementType);
                    }
                    name = typecheck_1.TypeChecker.mangledTypeName(typecheck_1.RestrictedType.strip(t)) + "." + name;
                }
                let wf = this.backend.declareFunction(name);
                this.funcs.set(e, wf);
            }
            else if (e instanceof typecheck_1.TemplateFunction) {
                // Do nothing by intention
            }
            else if (e instanceof typecheck_1.Variable) {
        ...
        t.returnType = typecheck_1.TypeChecker.t_void;
        ...
        for (let v of globals) {
            ...
            if ((this.tc.isStruct(v.type) || this.tc.isArray(v.type)) && this.isPureLiteral(v.type, v.node.rhs)) {
                ...
                if (v.isConst) {
                    g.isConstant = true;
                    g.constantValue = expr.constantValue;
                }
                else {
    ...
    getSSAType(t) {
        if (t == typecheck_1.TypeChecker.t_bool || t == typecheck_1.TypeChecker.t_uint8 || t == typecheck_1.TypeChecker.t_byte || t == typecheck_1.TypeChecker.t_void) {
            return "i8";
        }
        if (t == typecheck_1.TypeChecker.t_int8) {
            return "s8";
        }
        if (t == typecheck_1.TypeChecker.t_int16) {
            return "s16";
        }
        if (t == typecheck_1.TypeChecker.t_uint16) {
            return "i16";
        }
        if (t == typecheck_1.TypeChecker.t_int32) {
            return "s32";
        }
        if (t == typecheck_1.TypeChecker.t_uint32) {
            return "i32";
        }
        if (t == typecheck_1.TypeChecker.t_int64) {
            return "s64";
        }
        if (t == typecheck_1.TypeChecker.t_uint64) {
            return "i64";
        }
        if (t == typecheck_1.TypeChecker.t_float) {
            return "f32";
        }
        if (t == typecheck_1.TypeChecker.t_double) {
            return "f64";
        }
        if (t == typecheck_1.TypeChecker.t_rune) {
            return "i32";
        }
        if (t == typecheck_1.TypeChecker.t_int) {
            return "sint";
        }
        if (t == typecheck_1.TypeChecker.t_uint) {
            return "int";
        }
        if (t == typecheck_1.TypeChecker.t_char) {
            return "s8";
        }
        if (t == typecheck_1.TypeChecker.t_byte) {
            return "i8";
        }
        if (t instanceof typecheck_1.RestrictedType && t.elementType instanceof tc.PointerType) {
        ...
        if (t instanceof typecheck_1.RestrictedType && t.elementType instanceof tc.UnsafePointerType) {
            return new ssa.PointerType(this.getSSAType(t.elementType.elementType), true);
        }
        if (t instanceof tc.UnsafePointerType) {
            return new ssa.PointerType(this.getSSAType(t.elementType), this.tc.isConst(t.elementType));
        }
        if (t == typecheck_1.TypeChecker.t_string) {
            return "addr";
        }
        if (t == typecheck_1.TypeChecker.t_null) {
            return "addr";
        }
        if (t instanceof typecheck_1.SliceType) {
            if (t.mode == "local_reference") {
                return this.localSlicePointer;
            }
            return this.slicePointer;
        }
        if (t instanceof typecheck_1.MapType) {
            return "ptr";
        }
        if (t instanceof typecheck_1.StructType) {
            ...
            for (let i = 0; i < t.fields.length; i++) {
                ...
                if (t.extends && i == 0) {
                    for (let entry of ft.fieldOffsetsByName.entries()) {
        ...
        if (t instanceof typecheck_1.ArrayType) {
            let s = new ssa.StructType();
            s.name = t.name;
            s.addField("data", this.getSSAType(t.elementType), t.size);
            return s;
        }
        if (t instanceof typecheck_1.TupleType) {
            let s = new ssa.StructType();
            s.name = t.name;
            let i = 0;
    ...
    getSSAFunctionType(t) {
        ...
        if (t.returnType != typecheck_1.TypeChecker.t_void) {
            ftype.result = this.getSSAType(t.returnType);
        }
        if (t.hasEllipsis()) {
            ftype.ellipsisParam = this.getSSAType(t.lastParameter().type.arrayType.elementType);
        }
        return ftype;
    }
    processFunction(f, wf) {
        let vars = new Map();
        // Add global variables
        for (let e of this.globalVars.keys()) {
            vars.set(e, this.globalVars.get(e));
        ...
        for (let name of f.scope.elements.keys()) {
            ...
            if (e instanceof typecheck_1.FunctionParameter) {
        ...
        if (f.namedReturnVariables) {
            for (let name of f.scope.elements.keys()) {
                let e = f.scope.elements.get(name);
                if (e instanceof typecheck_1.Variable && e.isResult) {
        ...
        else if (f.type.returnType != typecheck_1.TypeChecker.t_void) {
        ...
        if (!f.type.returnType || f.type.returnType == typecheck_1.TypeChecker.t_void) {
            // Free all variables
    ...
    processScopeVariables(b, vars, scope) {
        // Declare variables
        for (let name of scope.elements.keys()) {
            let e = scope.elements.get(name);
            if (e instanceof typecheck_1.Variable) {
                if (e.isResult) {
                    continue;
                }
                else {
    ...
    freeScopeVariables(ignoreVariables, b, vars, scope) {
        // Declare variables
        for (let name of scope.elements.keys()) {
            let e = scope.elements.get(name);
            if ((e instanceof typecheck_1.Variable && !e.isResult) || (e instanceof typecheck_1.FunctionParameter && !e.isConst)) {
    ...
    processStatement(f, scope, snode, b, vars, blocks) {
        switch (snode.op) {
            case "comment":
                break;
            case "if":
                {
                    ...
                    let tmp = this.processExpression(f, snode.scope, snode.condition, b, vars, typecheck_1.TypeChecker.t_bool);
                    b.ifBlock(tmp);
                    ...
                    if (snode.rhs) { // Assignment of an expression value?
                        if (snode.lhs.op == "id") {
                            ...
                            if ((this.tc.isArray(element.type) || this.tc.isStruct(element.type)) && this.isPureLiteral(element.type, snode.rhs)) {
                                ...
                                if (element.isConst) {
                                    v.isConstant = true;
                                    v.constantValue = data.constantValue;
                                }
                                else {
                                ...
                                if (this.tc.isSafePointer(snode.lhs.type) && typecheck_1.TypeChecker.isReference(snode.lhs.type) && (typecheck_1.TypeChecker.isStrong(snode.rhs.type) || typecheck_1.TypeChecker.isUnique(snode.rhs.type) || !this.tc.isTakeExpression(snode.rhs))) {
                                ...
                                b.assign(v, "copy", v.type, [data]);
                                if (this.tc.isSlice(snode.lhs.type) && typecheck_1.TypeChecker.isReference(snode.lhs.type) && (typecheck_1.TypeChecker.isStrong(snode.rhs.type) || typecheck_1.TypeChecker.isUnique(snode.rhs.type) || !this.tc.isTakeExpression(snode.rhs))) {
                                    let st = this.getSSAType(snode.lhs.type);
                                    let arrayPointer;
                                ...
                                if ((snode.rhs.flags & ast_1.AstFlags.ZeroAfterAssignment) == ast_1.AstFlags.ZeroAfterAssignment || snode.rhs.op == "take") {
            ...
            case "=":
                {
                    if (snode.lhs.op == "tuple" || snode.lhs.op == "array" || snode.lhs.op == "object") {
                        var processAssignmentDestinations = (node, destinations) => {
                            if (node.op == "tuple") {
                                for (let p of node.parameters) {
                                    if (p.op == "tuple" || p.op == "array" || p.op == "object") {
                                        ...
                                        let dest = this.processLeftHandExpression(f, scope, p, b, vars);
                        ...
                        var processAssignment = (node, type, rhsIsTakeExpr, destinations, destCount, source) => {
                            if (node.op == "tuple") {
                                if (!(type instanceof typecheck_1.TupleType)) {
                                    throw "Implementation error";
                                }
                                let stype = this.getSSAType(type);
                                for (let i = 0; i < node.parameters.length; i++) {
                                    ...
                                    if (p.op == "tuple" || p.op == "array" || p.op == "object") {
                                        ...
                                        if (this.tc.isSafePointer(p.type) && typecheck_1.TypeChecker.isReference(p.type) && (typecheck_1.TypeChecker.isStrong(elementType) || typecheck_1.TypeChecker.isUnique(elementType) || !rhsIsTakeExpr)) {
                                            // Assigning to ~ptr means that the reference count needs to be increased unless the RHS is a take expressions which yields ownership
                                        ...
                                        if (this.tc.isSlice(p.type) && typecheck_1.TypeChecker.isReference(p.type) && (typecheck_1.TypeChecker.isStrong(elementType) || typecheck_1.TypeChecker.isUnique(elementType) || !rhsIsTakeExpr)) {
                                            let st = this.getSSAType(snode.lhs.type);
                                            let arrayPointer;
                                            if (dest instanceof ssa.Pointer) {
                        ...
                        if (this.isLeftHandSide(snode.rhs)) {
                            ...
                            val = this.processExpression(f, scope, snode.rhs, b, vars, snode.lhs.type);
                        }
                    ...
                    else if (snode.lhs.op == "[" && this.tc.stripType(snode.lhs.lhs.type) instanceof typecheck_1.MapType) {
                        ...
                        */
                        ...
                        let t = this.getSSAType(snode.rhs.type);
                        let dest = this.processLeftHandExpression(f, scope, snode.lhs, b, vars);
                        let rhs;
            ...
            case "<<=":
                {
                    ...
                    let tmp = this.processLeftHandExpression(f, scope, snode.lhs, b, vars);
                    let p1;
                    let dest;
                    ...
                    if (snode.lhs.type == typecheck_1.TypeChecker.t_string) {
                        ...
                        if (!this.disableNullCheck && !p2.isConstant) {
                        ...
                        this.callDestructorOnVariable(typecheck_1.TypeChecker.t_string, p1, b, true);
                        // Decref p2 if necessary
                        if (this.tc.isTakeExpression(snode.rhs)) {
                            this.callDestructorOnVariable(typecheck_1.TypeChecker.t_string, p2, b, true);
                        }
                        dest = ptr;
                    }
                    ...
                    else if (t instanceof typecheck_1.UnsafePointerType) {
            ...
            case "++":
                {
                    ...
                    let increment = 1;
                    if (t instanceof typecheck_1.UnsafePointerType) {
            ...
            case "for":
                {
                    ...
                    if (snode.condition && snode.condition.op == ";;" && snode.condition.lhs) {
                        ...
                        //
                        let t = typecheck_1.RestrictedType.strip(snode.condition.rhs.type);
                        //
                        ...
                        if (t instanceof typecheck_1.SliceType) {
                            ...
                            if (this.isLeftHandSide(snode.condition.rhs)) {
                                ...
                                sliceHeader = this.processExpression(f, snode.scope, snode.condition.rhs, b, vars, t);
                            }
                        ...
                        else if (t instanceof typecheck_1.ArrayType) {
                            // TODO: Incref
                            ...
                            if (this.isLeftHandSide(snode.condition.rhs)) {
                                ...
                                let arr = this.processExpression(f, snode.scope, snode.condition.rhs, b, vars, t);
                                ptr = b.assign(b.tmp(), "addr_of", "addr", [arr]);
                            }
                        }
                        else if (t == typecheck_1.TypeChecker.t_string) {
                            // TODO: Incref
                            ptr = this.processExpression(f, snode.scope, snode.condition.rhs, b, vars, typecheck_1.TypeChecker.t_string);
                        ...
                        if (snode.condition.lhs.op == "tuple") {
                            if (snode.condition.lhs.parameters[0].value != "_") {
                                // Initialize the counter with 0                            
                                let element = snode.scope.resolveElement(snode.condition.lhs.parameters[0].value);
                            ...
                            if (snode.condition.lhs.parameters[1].value != "_") {
                                let valElement = snode.scope.resolveElement(snode.condition.lhs.parameters[1].value);
                                val = vars.get(valElement);
                            }
                        }
                        else {
                            if (snode.condition.lhs.value != "_") {
                                let element = snode.scope.resolveElement(snode.condition.lhs.value);
                    ...
                    if (snode.condition) {
                        if (snode.condition.op == ";;") {
                            if (snode.condition.condition) {
                                let tmp = this.processExpression(f, snode.scope, snode.condition.condition, b, vars, typecheck_1.TypeChecker.t_bool);
                            ...
                            let t = typecheck_1.RestrictedType.strip(snode.condition.rhs.type);
                            if (t instanceof typecheck_1.SliceType || t instanceof typecheck_1.ArrayType) {
                                // TODO: null-check
            ...
            case "return":
                {
                    ...
                    if (snode.lhs) {
                        ...
                        if ((snode.lhs.flags & ast_1.AstFlags.ZeroAfterAssignment) == ast_1.AstFlags.ZeroAfterAssignment && snode.lhs.op == "id") {
                            let e = scope.resolveElement(snode.lhs.value);
                            if (e instanceof typecheck_1.FunctionParameter || (e instanceof typecheck_1.Variable && !e.isGlobal)) {
                            ...
                            if (e instanceof typecheck_1.FunctionParameter && e.isConst) {
                                forceIncref = true;
                            }
                        }
                        if (!doNotZero && ((snode.lhs.flags & ast_1.AstFlags.ZeroAfterAssignment) == ast_1.AstFlags.ZeroAfterAssignment || snode.lhs.op == "take")) {
                        ...
                        // Reference counting for pointers
                        if (this.tc.isSafePointer(targetType) && typecheck_1.TypeChecker.isReference(targetType) && (typecheck_1.TypeChecker.isStrong(snode.lhs.type) || typecheck_1.TypeChecker.isUnique(snode.lhs.type) || !this.tc.isTakeExpression(snode.lhs) || forceIncref)) {
                            // Assigning to ~ptr means that the reference count needs to be increased unless the RHS is a take expressions which yields ownership
                            data = b.assign(b.tmp(), "incref", "addr", [data]);
                        }
                        ...
                        // Reference counting for slices
                        if (this.tc.isSlice(targetType) && typecheck_1.TypeChecker.isReference(targetType) && (typecheck_1.TypeChecker.isStrong(snode.lhs.type) || typecheck_1.TypeChecker.isUnique(snode.lhs.type) || !this.tc.isTakeExpression(snode.rhs))) {
                            let st = this.getSSAType(snode.lhs.type);
                            let arrayPointer;
                            if (rhs instanceof ssa.Pointer) {
                                arrayPointer = b.assign(b.tmp(), "load", "addr", [rhs.variable, rhs.offset + st.fieldOffset("array_ptr")]);
                        ...
                        if (!snode.lhs) {
                            if (f.namedReturnVariables) {
                                ...
                                for (let key of f.scope.elements.keys()) {
                                    ...
                                    if (v instanceof typecheck_1.Variable && v.isResult) {
            ...
            case "take":
                {
                    ...
                    let src = this.processLeftHandExpression(f, scope, snode.lhs, b, vars);
            ...
            case "copy":
                {
                    ...
                    if (!(objType instanceof typecheck_1.SliceType)) {
                        throw "Implementation error";
                    }
                    let elementType = this.getSSAType(typecheck_1.RestrictedType.strip(objType.getElementType()));
                    ...
                    if (this.isLeftHandSide(snode.lhs)) {
                        ...
                        head_addr = this.processExpression(f, scope, snode.lhs, b, vars, objType);
                    }
                    let dest_data_ptr;
                    let dest_count;
                    ...
                    if (this.isLeftHandSide(snode.rhs)) {
                        ...
                        head_addr = this.processExpression(f, scope, snode.rhs, b, vars, objType);
                    }
                    let src_data_ptr;
                    let src_count;
                    if (head_addr instanceof ssa.Variable) {
    ...
    processLeftHandExpression(f, scope, enode, b, vars) {
        switch (enode.op) {
            case "id":
                {
            ...
            case "[":
                {
                    ...
                    if (ltype instanceof typecheck_1.UnsafePointerType) {
                        let ptr = this.processExpression(f, scope, enode.lhs, b, vars, ltype);
                        let index = this.processExpression(f, scope, enode.rhs, b, vars, typecheck_1.TypeChecker.t_int);
                    ...
                    else if (ltype instanceof typecheck_1.SliceType) {
                        ...
                        if (this.isLeftHandSide(enode.lhs)) {
                            ...
                            head_addr = this.processExpression(f, scope, enode.lhs, b, vars, ltype);
                        }
                        let data_ptr;
                        let len;
                        ...
                        if (enode.rhs.op == "int") {
                            ...
                            index = this.processExpression(f, scope, enode.rhs, b, vars, typecheck_1.TypeChecker.t_int);
                        }
                    ...
                    else if (ltype instanceof typecheck_1.ArrayType) {
                        let size = ssa.alignedSizeOf(this.getSSAType(ltype.elementType));
                        let ptr;
                        if (this.isLeftHandSide(enode.lhs)) {
                            ...
                            ptr = this.processExpression(f, scope, enode.lhs, b, vars, ltype);
                        }
                    ...
                    else if (ltype instanceof typecheck_1.TupleType) {
                        let ptr;
                        if (this.isLeftHandSide(enode.lhs)) {
                            ptr = this.processLeftHandExpression(f, scope, enode.lhs, b, vars);
                        }
            ...
            case ".":
                {
                    ...
                    // Note: This code implements the non-left-hand cases as well to avoid duplicating code
                    if (t instanceof typecheck_1.PointerType || t instanceof typecheck_1.UnsafePointerType) {
                        let ptr = this.processExpression(f, scope, enode.lhs, b, vars, t);
                        if (t instanceof typecheck_1.PointerType && !this.disableNullCheck && !this.isThis(ptr)) {
                        ...
                        if (elementType instanceof typecheck_1.RestrictedType) {
                            elementType = elementType.elementType;
                        }
                        if (elementType instanceof typecheck_1.StructType) {
                            let s = this.getSSAType(elementType);
                            if (ptr instanceof ssa.Variable) {
                    ...
                    else if (t instanceof typecheck_1.PackageType) {
                        throw "TODO";
                    }
                    else if (t instanceof typecheck_1.StructType) {
                        ...
                        if (this.isLeftHandSide(enode.lhs)) {
                            ...
                            left = this.processExpression(f, scope, enode.lhs, b, vars, t);
                        }
                        let s = this.getSSAType(enode.lhs.type);
    ...
    processPureLiteralInternal(n, buf) {
        if (n.type == typecheck_1.TypeChecker.t_bool) {
            buf.push(n.value == "true" ? 1 : 0);
        }
        else if (n.type == typecheck_1.TypeChecker.t_uint8) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_uint16) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_uint32 || n.type == typecheck_1.TypeChecker.t_rune) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_uint64) {
            // TODO large numbers
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_int8) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_int16) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_int32) {
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_int64) {
            // TODO large numbers
            buf.push(parseInt(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_float) {
            buf.push(parseFloat(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_double) {
            buf.push(parseFloat(n.value));
        }
        else if (n.type == typecheck_1.TypeChecker.t_string) {
            buf.push(n.value);
        }
            ...
            if (n.parameters) {
                ...
                if (count > 0) {
                    let t = typecheck_1.RestrictedType.strip(n.type);
            ...
            for (let f of this.tc.stripType(n.type).fields) {
                let found = false;
                if (n.parameters) {
                    for (let p of n.parameters) {
    ...
    */
    isLeftHandSide(node) {
        if (node.op == "id") {
            ...
            if (node.lhs.type instanceof typecheck_1.PointerType || node.lhs.type instanceof typecheck_1.UnsafePointerType) {
                return true;
            }
            return this.isLeftHandSide(node.lhs);
        }
        else if (node.op == "[") {
            if (node.lhs.type instanceof typecheck_1.UnsafePointerType || node.lhs.type instanceof typecheck_1.SliceType) {
                return true;
            }
            return this.isLeftHandSide(node.lhs);
        }
    ...
    createInterfaceTable(iface, s) {
        let table = [];
        let dtr = this.generateStructDestructor(s);
        for (let m of iface.sortMethodNames()) {
            ...
            let methodObjType = typecheck_1.RestrictedType.strip(typecheck_1.RestrictedType.strip(method.objectType).elementType);
            ...
            if (!(f instanceof typecheck_1.Function)) {
                throw "Implementation error";
            }
    ...
    createInterfaceDescriptor(ifaceType, structType) {
        // Assign a pointer to some struct to a pointer to some interface? -> create an ifaceHeader instance
        let typecode = typecheck_1.RestrictedType.strip(ifaceType).toTypeCodeString() + "::" + typecheck_1.RestrictedType.strip(structType).toTypeCodeString();
        let descriptor;
    ...
    autoConvertData(data, targetType, fromType, b) {
        let v;
        ...
        if (this.tc.isSlice(fromType) && !typecheck_1.TypeChecker.isLocalReference(fromType) && this.tc.isSlice(targetType) && typecheck_1.TypeChecker.isLocalReference(targetType)) {
            ...
            let structType = typecheck_1.RestrictedType.strip(typecheck_1.RestrictedType.strip(fromType).elementType);
            let ifaceType = typecheck_1.RestrictedType.strip(typecheck_1.RestrictedType.strip(targetType).elementType);
            if (!(structType instanceof typecheck_1.StructType)) {
                throw "Implementation error";
            }
            if (!(ifaceType instanceof typecheck_1.InterfaceType)) {
                throw "Implementation error";
            }
    ...
    */
    processExpression(f, scope, enode, b, vars, targetType) {
        switch (enode.op) {
            case "null":
                return 0;
            case "int":
            ...
            case "float":
                {
                    let v = new ssa.Variable();
                    if (enode.type == typecheck_1.TypeChecker.t_float) {
                        v.type = "f32";
                    }
                    else {
                        v.type = "f64";
            ...
            case "str":
                {
                    if (this.tc.isStringLiteralType(enode.type)) {
                        let idx;
                        let sl = typecheck_1.RestrictedType.strip(enode.type);
            ...
            case "object":
                {
                    ...
                    if (t instanceof typecheck_1.PointerType) {
                        ...
                        else if (t instanceof typecheck_1.MapType) {
                            /*
            ...
            case "tuple":
                {
                    ...
                    for (let i = 0; i < enode.parameters.length; i++) {
                        let v = this.processLiteralArgument(f, scope, enode.parameters[i], t.types[i], b, vars);
                        args.push(v);
                    }
                    return b.assign(b.tmp(), "struct", st, args);
                }
                    ...
                    if (t instanceof typecheck_1.SliceType) {
                        ...
                        for (let i = 0; i < enode.parameters.length; i++) {
                            ...
                            if (p.op == "unary..." && typeof (count) == "number") {
                                count--;
                                let dynCount = this.processExpression(f, scope, p.rhs, b, vars, typecheck_1.TypeChecker.t_int);
            ...
            case "<":
                {
                    let t = this.tc.stripType(enode.lhs.type);
                    if (t == typecheck_1.TypeChecker.t_float || t == typecheck_1.TypeChecker.t_double || t == typecheck_1.TypeChecker.t_string) {
                        return this.processCompare("lt", f, scope, enode, b, vars);
                    }
                    if (!(t instanceof typecheck_1.UnsafePointerType) && this.isSigned(t)) {
            ...
            case "+":
                {
                    ...
                    if (t == typecheck_1.TypeChecker.t_string) {
                        ...
                        if (!this.disableNullCheck && !p1.isConstant) {
                            let cond = b.assign(b.tmp(), "eq", "addr", [p1, 0]);
                            b.ifBlock(cond);
                            b.assign(null, "trap", null, []);
                            b.end();
                    ...
                    let p2;
                    if (t instanceof typecheck_1.UnsafePointerType) {
                        p2 = this.processExpression(f, scope, enode.rhs, b, vars, typecheck_1.TypeChecker.t_int);
                        let estorage = this.getSSAType(t.elementType);
                        let size = ssa.sizeOf(estorage);
                        if (size > 1) {
                            p2 = b.assign(b.tmp(), "mul", "i32", [p2, size]);
            ...
            case "/":
                {
                    ...
                    let opcode = this.isSigned(t) ? "div_s" : "div_u";
                    return b.assign(b.tmp(), opcode, storage, [p1, p2]);
                }
            case "%":
                {
                    ...
                    let p2 = this.processExpression(f, scope, enode.rhs, b, vars, t);
                    let storage = this.getSSAType(t);
                    let opcode = this.isSigned(t) ? "rem_s" : "rem_u";
                    return b.assign(b.tmp(), opcode, storage, [p1, p2]);
                }
            case "|":
            case "&":
            case "^":
                {
                    let t = this.tc.stripType(enode.type);
                    let opcode = enode.op == "|" ? "or" : (enode.op == "&" ? "and" : "xor");
                    let p1 = this.processExpression(f, scope, enode.lhs, b, vars, t);
                    let p2 = this.processExpression(f, scope, enode.rhs, b, vars, t);
                    let storage = this.getSSAType(t);
                    return b.assign(b.tmp(), opcode, storage, [p1, p2]);
            ...
            case "unary-":
                {
                    ...
                    let p = this.processExpression(f, scope, enode.rhs, b, vars, t);
                    let storage = this.getSSAType(t);
                    if (t == typecheck_1.TypeChecker.t_float || t == typecheck_1.TypeChecker.t_double) {
            ...
            case "unary*":
                {
                    ...
                    else if (t instanceof typecheck_1.PointerType) {
                        let storage = this.getSSAType(t.elementType);
                        return b.assign(b.tmp(), "load", storage, [p, 0]);
                    }
                    throw "Implementation error";
            ...
            case "spawn":
                {
                    ...
                    if (striplhs instanceof typecheck_1.FunctionType && striplhs.callingConvention == "system" && striplhs.name == "remove") {
                        /*
                    ...
                    else if (striplhs instanceof typecheck_1.FunctionType && striplhs.callingConvention == "system") {
                        t = striplhs;
                    }
                    else if (enode.lhs.op == "id") {
                        // Calling a named function
                        let e = scope.resolveElement(enode.lhs.value);
                        if (e instanceof typecheck_1.TemplateFunction) {
                            if (!(enode.lhs.type instanceof typecheck_1.TemplateFunctionType)) {
                                throw "Implementation error";
                            }
                            let name = e.type.pkg.pkgPath + "/" + enode.lhs.value + typecheck_1.TypeChecker.mangleTemplateParameters(enode.lhs.type.templateParameterTypes);
                        ...
                        if (!(tmplFunc instanceof typecheck_1.TemplateFunction)) {
                            throw "Implementation error";
                        }
                        let types = [];
                        ...
                        let name = tmplFunc.type.pkg.pkgPath + "/" + enode.lhs.lhs.value + typecheck_1.TypeChecker.mangleTemplateParameters(types);
                    ...
                    else if (enode.lhs.op == "." && enode.lhs.lhs.type instanceof typecheck_1.PackageType) {
                        ...
                        if (e instanceof typecheck_1.TemplateFunction) {
                            ...
                            let name = e.type.pkg.pkgPath + "/" + enode.lhs.name.value + typecheck_1.TypeChecker.mangleTemplateParameters(enode.lhs.type.templateParameterTypes);
                            e = scope.resolveElement(name);
                        }
                        if (!(e instanceof typecheck_1.Function)) {
                            throw "Implementation error";
                        ...
                        let objType;
                        if (ltype instanceof typecheck_1.PointerType) {
                            objType = typecheck_1.RestrictedType.strip(ltype.elementType);
                            if (!(objType instanceof typecheck_1.InterfaceType)) {
                        ...
                        else if (ltype instanceof typecheck_1.UnsafePointerType) {
                            objType = typecheck_1.RestrictedType.strip(ltype.elementType);
                            objPtr = this.processExpression(f, scope, enode.lhs.lhs, b, vars, ltype);
                        }
                        else if (ltype instanceof typecheck_1.StructType) {
                            objType = ltype;
                        ...
                        if (objType instanceof typecheck_1.StructType) {
                            let method = objType.method(enode.lhs.name.value);
                            let methodObjType = typecheck_1.RestrictedType.strip(method.objectType);
                            methodObjType = typecheck_1.RestrictedType.strip(method.objectType);
                            if (methodObjType instanceof typecheck_1.PointerType) {
                                methodObjType = typecheck_1.RestrictedType.strip(methodObjType.elementType);
                            }
                            let methodName = typecheck_1.TypeChecker.mangledTypeName(methodObjType) + "." + enode.lhs.name.value;
                        ...
                        else if (objType instanceof typecheck_1.InterfaceType) {
                            let iface;
                            if (this.isLeftHandSide(enode.lhs.lhs)) {
                                iface = this.processLeftHandExpression(f, scope, enode.lhs.lhs, b, vars);
                            }
                            else {
                                iface = this.processExpression(f, scope, enode.lhs.lhs, b, vars, ltype);
                            }
                            let table;
                    ...
                    if (t.hasEllipsis() && (enode.parameters.length != t.parameters.length || enode.parameters[enode.parameters.length - 1].op != "unary...")) {
                        ...
                        let elementType = this.getSSAType(t.lastParameter().type.getElementType());
                        ...
                        for (let i = normalParametersCount; i < enode.parameters.length; i++, offset += elementSize) {
                            let v = this.processExpression(f, scope, enode.parameters[i], b, vars, t.lastParameter().type.getElementType());
                        ...
                        for (let i = 0; i < enode.parameters.length; i++) {
                            ...
                            let rhs;
                            if ((vnode.flags & ast_1.AstFlags.ZeroAfterAssignment) == ast_1.AstFlags.ZeroAfterAssignment || vnode.op == "take") {
                    ...
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.abs32) {
                        result = b.assign(b.tmp(), "abs", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.abs64) {
                        result = b.assign(b.tmp(), "abs", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.floor32) {
                        result = b.assign(b.tmp(), "floor", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.floor64) {
                        result = b.assign(b.tmp(), "floor", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.ceil32) {
                        result = b.assign(b.tmp(), "ceil", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.ceil64) {
                        result = b.assign(b.tmp(), "ceil", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.sqrt32) {
                        result = b.assign(b.tmp(), "sqrt", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.sqrt64) {
                        result = b.assign(b.tmp(), "sqrt", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.trunc32) {
                        result = b.assign(b.tmp(), "trunc", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.trunc64) {
                        result = b.assign(b.tmp(), "trunc", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.nearest32) {
                        result = b.assign(b.tmp(), "nearest", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.nearest64) {
                        result = b.assign(b.tmp(), "nearest", "f64", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.copysign32) {
                        result = b.assign(b.tmp(), "copysign", "f32", [args[0]]);
                    }
                    else if (t.callingConvention == "system" && t.systemCallType == pkg_1.SystemCalls.copysign64) {
            ...
            case ":":
                {
                    ...
                    if (enode.parameters[0]) {
                        if (enode.parameters[0].op == "int") {
                            ...
                            index1 = this.processExpression(f, scope, enode.parameters[0], b, vars, typecheck_1.TypeChecker.t_int);
                        }
                    }
                    let index2 = 0;
                    if (enode.parameters[1]) {
                        if (enode.parameters[1].op == "int") {
                            ...
                            index2 = this.processExpression(f, scope, enode.parameters[1], b, vars, typecheck_1.TypeChecker.t_int);
                        }
                    }
                    let t = this.tc.stripType(enode.lhs.type);
                    if (t instanceof typecheck_1.UnsafePointerType) {
                        if (!typecheck_1.TypeChecker.isLocalReference(enode.type)) {
                            throw "Implementation error";
                        }
                    ...
                    else if (t instanceof typecheck_1.SliceType) {
                        ...
                        if (this.isLeftHandSide(enode.lhs)) {
                            ...
                            head_addr = this.processExpression(f, scope, enode.lhs, b, vars, t);
                        }
                        let data_ptr;
                        let len;
                        let array_ptr;
                        ...
                        let ptr = this.processExpression(f, scope, enode.lhs, b, vars, typecheck_1.TypeChecker.t_string);
            ...
            case "[":
                {
                    ...
                    let ptr = this.processLeftHandExpression(f, scope, enode, b, vars);
                    let storage = this.getSSAType(enode.type);
            ...
            case ".":
                {
                    ...
                    if (t instanceof typecheck_1.StructType) {
                        // It is a value, i.e. not a pointer to a value
                        let left = this.processExpression(f, scope, enode.lhs, b, vars, t);
                        let memberType = this.getSSAType(enode.type);
                        let structType = this.getSSAType(enode.lhs.type);
            ...
            case "is":
                {
                    let rtype = typecheck_1.RestrictedType.strip(enode.rhs.type);
                    let ltype = typecheck_1.RestrictedType.strip(enode.lhs.type);
                    if (this.tc.isStringOrType(enode.lhs.type)) {
                        ...
                        if (this.isLeftHandSide(enode.lhs)) {
                            ...
                            ifaceAddr = this.processExpression(f, scope, enode.lhs, b, vars, enode.lhs.type);
                        }
                        let ifaceType = typecheck_1.RestrictedType.strip(ltype.elementType);
                        if (!(ifaceType instanceof typecheck_1.InterfaceType)) {
                            throw "Implementation error";
                        }
                        let structType = typecheck_1.RestrictedType.strip(rtype.elementType);
            ...
            case "typeCast":
                {
                    ...
                    if ((t == typecheck_1.TypeChecker.t_float || t == typecheck_1.TypeChecker.t_double) && this.tc.isIntNumber(t2)) {
                        // Ints can be converted to floats
                        let to = this.getSSAType(t);
                        let op;
                        if (t2 == typecheck_1.TypeChecker.t_uint64) {
                            op = "convert64_u";
                        }
                        else if (t2 == typecheck_1.TypeChecker.t_int64) {
                            op = "convert64_s";
                        }
                        else {
                    ...
                    else if (this.tc.isIntNumber(t) && (t2 == typecheck_1.TypeChecker.t_float || t2 == typecheck_1.TypeChecker.t_double)) {
                        // Floats can be converted to ints
                        let to = this.getSSAType(t);
                        let op;
                        if (t2 == typecheck_1.TypeChecker.t_double) {
                            op = "trunc64";
                        }
                        else {
                            op = "trunc32";
                    ...
                    else if (t == typecheck_1.TypeChecker.t_float && t2 == typecheck_1.TypeChecker.t_double) {
                        // Doubles can be converted to floats
                        return b.assign(b.tmp(), "demote", "f32", [expr]);
                    }
                    else if (t == typecheck_1.TypeChecker.t_double && t2 == typecheck_1.TypeChecker.t_float) {
                        // Floats can be converted to doubles
                        return b.assign(b.tmp(), "promote", "f64", [expr]);
                    }
                    else if (this.tc.isIntNumber(t) && t2 instanceof typecheck_1.UnsafePointerType) {
                    ...
                    else if (this.tc.checkIsIntNumber(enode.rhs, false) && t instanceof typecheck_1.UnsafePointerType) {
                    ...
                    else if (t instanceof typecheck_1.UnsafePointerType && (t2 instanceof typecheck_1.UnsafePointerType || t2 instanceof typecheck_1.PointerType || t2 == typecheck_1.TypeChecker.t_string)) {
                        // Convert pointer or string to unsafe pointer
                        return expr;
                    }
                    else if (t == typecheck_1.TypeChecker.t_string && t2 instanceof typecheck_1.UnsafePointerType) {
                        // Convert unsafe pointer to string
                        return expr;
                    }
                    else if (t == typecheck_1.TypeChecker.t_string && t2 instanceof typecheck_1.SliceType) {
                        ...
                        if (this.tc.isTakeExpression(enode.rhs)) {
                            this.callDestructorOnVariable(t2, expr, b, true);
                        }
                        return newptr;
                    }
                    else if ((t == typecheck_1.TypeChecker.t_bool || t == typecheck_1.TypeChecker.t_rune || this.tc.isIntNumber(t)) && (t2 == typecheck_1.TypeChecker.t_bool || t2 == typecheck_1.TypeChecker.t_rune || this.tc.checkIsIntNumber(enode.rhs, false))) {
                    ...
                    else if (t instanceof typecheck_1.PointerType && t2 instanceof typecheck_1.UnsafePointerType) {
                        return expr;
                    }
                    else if (t instanceof typecheck_1.SliceType && t.getElementType() == typecheck_1.TypeChecker.t_byte && t2 == typecheck_1.TypeChecker.t_string) {
                    ...
                    else if (t2 == typecheck_1.TypeChecker.t_null) {
            ...
            case "take":
                {
                    ...
                    let src = this.processLeftHandExpression(f, scope, enode.lhs, b, vars);
                    let pointer;
                    if (src instanceof ssa.Pointer) {
            ...
            case "len":
                {
                    let objType = typecheck_1.RestrictedType.strip(enode.lhs.type);
                    if (objType == typecheck_1.TypeChecker.t_string) {
                        let s = this.processExpression(f, scope, enode.lhs, b, vars, typecheck_1.TypeChecker.t_string);
                        return b.assign(b.tmp(), "len_str", "sint", [s]);
                    }
                    else if (objType instanceof typecheck_1.SliceType) {
                        // Get the address of the SliceHead. Either compute it from a left-hand-side expression or put it on the stack first
                        let head_addr;
                        if (this.isLeftHandSide(enode.lhs)) {
                            head_addr = this.processLeftHandExpression(f, scope, enode.lhs, b, vars);
                            ...
                            head_addr = this.processExpression(f, scope, enode.lhs, b, vars, objType);
                        }
                        if (head_addr instanceof ssa.Variable) {
                            if (objType.mode == "local_reference") {
                    ...
                    else if (objType instanceof typecheck_1.ArrayType) {
                        return objType.size;
                    }
                    // TODO: Map
                    throw "Implementation error";
            ...
            case "append":
                {
                    ...
                    let head_addr;
                    if (enode.op == "append") {
                        head_addr = this.processExpression(f, scope, enode.parameters[0], b, vars, enode.parameters[0].type);
                    }
                    else {
                    ...
                    for (let i = 1; i < enode.parameters.length; i++) {
                        ...
                        if (p.op == "unary...") {
                            ...
                            if (this.isLeftHandSide(p)) {
                                ...
                                head_addr = this.processExpression(f, scope, p, b, vars, objType);
                            }
                            let src_data_ptr;
                            let src_count;
                            if (head_addr instanceof ssa.Variable) {
    ...
    processFillZeros(target, type, b) {
        // Fill the RHS with zeros
    ...
    processCompare(opcode, f, scope, enode, b, vars) {
        let t = this.tc.stripType(enode.lhs.type);
        let p1 = this.processExpression(f, scope, enode.lhs, b, vars, t);
        let p2 = this.processExpression(f, scope, enode.rhs, b, vars, t);
        if (t == typecheck_1.TypeChecker.t_string) {
    ...
    isSigned(t) {
        ...
        if (t == typecheck_1.TypeChecker.t_int8 || t == typecheck_1.TypeChecker.t_int16 || t == typecheck_1.TypeChecker.t_int32 || t == typecheck_1.TypeChecker.t_int64 || t == typecheck_1.TypeChecker.t_float || t == typecheck_1.TypeChecker.t_double) {
            return true;
        }
        if (t == typecheck_1.TypeChecker.t_uint8 || t == typecheck_1.TypeChecker.t_uint16 || t == typecheck_1.TypeChecker.t_uint32 || t == typecheck_1.TypeChecker.t_uint64) {
            return false;
        }
    ...
    isPureLiteral(t, n) {
        t = typecheck_1.RestrictedType.strip(t);
        if (t instanceof typecheck_1.InterfaceType) {
            return false;
        }
        switch (n.op) {
                    ...
                    if (this.tc.isArray(t)) {
                        if (n.parameters) {
                            for (let p of n.parameters) {
                                ...
                                else if (!this.isPureLiteral(t.elementType, p)) {
                                    return false;
                                }
                            }
                        }
            ...
            case "tuple":
                {
                    let i = 0;
                    for (let p of n.parameters) {
                        if (!this.isPureLiteral(t.types[i], p)) {
                            return false;
                        }
                    }
                    return true;
    ...
    generateArrayDestructor(t) {
        let tc = this.typecode(t).toString();
        let elementType = typecheck_1.RestrictedType.strip(t.elementType);
        let bf = this.destructors.get(tc);
        if (bf) {
            return bf;
        }
     ...
     */
    callDestructor(typ, pointer, b, avoidNullCheck, free) {
        let t = typecheck_1.RestrictedType.strip(typ);
        if (!avoidNullCheck) {
            let cond;
            if (t instanceof typecheck_1.InterfaceType) {
        ...
        let dtr;
        if (!this.tc.isPureValue(typ) && !typecheck_1.TypeChecker.isLocalReference(typ)) {
            if (t instanceof typecheck_1.InterfaceType) {
                if (free != "decref") {
                    let realPointer = b.assign(b.tmp(), "member", "addr", [pointer, this.ifaceHeader.fieldIndexByName("pointer")]);
            ...
            else if (t instanceof typecheck_1.TupleType) {
        ...
        if (free == "free") {
            ...
            else if (t instanceof typecheck_1.InterfaceType) {
                let realPointer = b.assign(b.tmp(), "member", "addr", [pointer, this.ifaceHeader.fieldIndexByName("pointer")]);
    ...
    callDestructorOnPointer(type, pointer, b) {
        let t = typecheck_1.RestrictedType.strip(type);
        if (this.tc.isPureValue(t)) {
            return;
        }
        if (t instanceof typecheck_1.PointerType && (t.mode == "strong" || t.mode == "unique")) {
        ...
        else if (t instanceof typecheck_1.PointerType && (t.mode == "reference")) {
            let v = b.assign(b.tmp(), "load", this.getSSAType(type), [pointer.variable, pointer.offset]);
        ...
        else if (t instanceof typecheck_1.ArrayType || t instanceof typecheck_1.TupleType || t instanceof typecheck_1.StructType || t instanceof typecheck_1.SliceType) {
    ...
    callDestructorOnVariable(type, v, b, avoidNullCheck = false) {
        let t = typecheck_1.RestrictedType.strip(type);
        if (this.tc.isPureValue(t)) {
            return;
        }
    ...
    functionArgumentIncref(rhs, rhsNode, rhsData, targetType, scope, b) {
        let decrefVar;
        if (this.tc.isSafePointer(targetType) && (typecheck_1.TypeChecker.isLocalReference(targetType) || typecheck_1.TypeChecker.isReference(targetType))) {
        ...
        else if (this.tc.isSlice(targetType) && (typecheck_1.TypeChecker.isLocalReference(targetType) || typecheck_1.TypeChecker.isReference(targetType))) {
            let result = this.functionArgumentIncrefIntern(rhsNode, scope);
            if (result[0] != "no") {
                let st = this.getSSAType(rhsNode.type);
                let arrayPointer;
                if (rhs instanceof ssa.Pointer) {
                    arrayPointer = b.assign(b.tmp(), "load", "addr", [rhs.variable, rhs.offset + st.fieldOffset("array_ptr")]);
                }
        ...
        if ((rhsNode.flags & ast_1.AstFlags.ZeroAfterAssignment) == ast_1.AstFlags.ZeroAfterAssignment || rhsNode.op == "take") {
            if (!(rhs instanceof ssa.Variable) && !(rhs instanceof ssa.Pointer)) {
                throw "Implementation error";
            }
    ...
    functionArgumentDecref(decrefVar, rhsNode, targetType, scope, b) {
        if (this.tc.isSafePointer(targetType) && (typecheck_1.TypeChecker.isLocalReference(targetType) || typecheck_1.TypeChecker.isReference(targetType))) {
            let result = this.functionArgumentIncrefIntern(rhsNode, scope);
            if (result[0] != "no") {
            ...
            let result = this.functionArgumentIncrefIntern(rhsNode, scope);
            if (result[0] != "no") {
                let sliceType = typecheck_1.RestrictedType.strip(rhsNode.type);
                let arrayType = typecheck_1.RestrictedType.strip(sliceType);
     ...
     */
    functionArgumentIncrefIntern(enode, scope) {
        if (typecheck_1.TypeChecker.isLocalReference(enode.type)) {
            ...
            case ".":
                {
                    ...
                    let type = typecheck_1.RestrictedType.strip(enode.lhs.type);
                    if (this.tc.isStruct(type) || this.tc.isUnsafePointer(type)) {
                        return lhs;
                    }
                    if (type instanceof typecheck_1.PointerType && (type.mode == "unique" || type.mode == "strong") && lhs[0] == "no" && lhs[1].localReferenceCount == 0) {
            ...
            case "[":
                {
                    ...
                    if (type instanceof typecheck_1.SliceType && (type.mode == "unique" || type.mode == "strong") && lhs[0] == "no" && lhs[1].localReferenceCount == 0) {
                        return ["one_indirection", lhs[1]];
                    }
                    return ["yes", null];
                }
            ...
            case "id":
                {
                    ...
                    if ((e instanceof typecheck_1.Variable || e instanceof typecheck_1.FunctionParameter)) {
                        return ["no", e];
                    }
                    return ["yes", null];
                }
    ...
    processLiteralArgument(f, scope, rhsNode, targetType, b, vars) {
        let rhs;
        ...
        // Reference counting for pointers
        if (this.tc.isSafePointer(targetType) && typecheck_1.TypeChecker.isReference(targetType) && (typecheck_1.TypeChecker.isStrong(rhsNode.type) || typecheck_1.TypeChecker.isUnique(rhsNode.type) || !this.tc.isTakeExpression(rhsNode))) {
            // Assigning to ~ptr means that the reference count needs to be increased unless the RHS is a take expressions which yields ownership
            data = b.assign(b.tmp(), "incref", "addr", [data]);
        }
        ...
        // Reference counting for slices
        if (this.tc.isSlice(targetType) && typecheck_1.TypeChecker.isReference(targetType) && (typecheck_1.TypeChecker.isStrong(rhsNode.type) || typecheck_1.TypeChecker.isUnique(rhsNode.type) || !this.tc.isTakeExpression(rhsNode))) {
            let st = this.getSSAType(targetType);
            let arrayPointer;
            if (rhs instanceof ssa.Pointer) {
                arrayPointer = b.assign(b.tmp(), "load", "addr", [rhs.variable, rhs.offset + st.fieldOffset("array_ptr")]);
            }
     ...
     */
    loadPackage(pkgPath, loc) {
        let p = pkg_2.Package.resolve(pkgPath, loc);
        pkg_2.Package.checkTypesForPackages();
        return p;
    }
    loadFunction(pkgPath, name, loc) {
        let p = this.loadPackage(pkgPath, loc);
        let f = p.scope.elements.get(name);
        if (!f || !(f instanceof typecheck_1.Function)) {
