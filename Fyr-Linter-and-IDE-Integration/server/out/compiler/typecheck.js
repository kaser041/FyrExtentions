...
exports.ImportedPackage = ImportedPackage;
...
class Scope {
    constructor(parent) {
        this.canonicalGroups = new Map();
        this.unavailableGroups = new Set();
     ...
     * @param element
     */
    registerType(name, type, loc = null) {
     ...
     * @param element
     */
    registerElement(name, element, loc = null) {
        if (this.elements.has(name)) {
            // TODO: Output file name
    ...
    joinGroups(group1, group2, loc, doThrow) {
        if (!group1) {
            if (!group2) {
...
class InterfaceType extends Type {
    constructor() {
        ...
        this.sortedMethodNames = [];
        // Required during recursive checking
        this._markChecked = false;
    }
    getAllMethods(map) {
        if (!map) {
            map = new Map();
...
class StructType extends Type {
    ...
    field(name, ownFieldsOnly = false) {
    ...
    toTypeCodeString() {
        ...
        let str = "struct{";
        str += this.fields.map(function (f) { return f.toTypeCodeString(); }).join(",");
        str += "}";
        return str;
    }
    getAllMethodsAndFields(map) {
        if (!map) {
            map = new Map();
        }
        for (let key of this.methods.keys()) {
 ...
 */
class TemplateFunctionType extends FunctionType {
    constructor() {
        super();
        this.templateParameterTypes = [];
    }
...
exports.TemplateFunctionType = TemplateFunctionType;
/**
 * TemplateStructType is the instance of a TemplateType.
 */
class TemplateStructType extends StructType {
    constructor() {
        super();
        this.templateParameterTypes = [];
    }
...
exports.TemplateStructType = TemplateStructType;
/**
 * TemplateInterfaceType is the instance of a TemplateType.
 */
class TemplateInterfaceType extends InterfaceType {
    constructor() {
        super();
        this.templateParameterTypes = [];
    }
...
exports.TemplateInterfaceType = TemplateInterfaceType;
class PointerType extends Type {
    constructor(elementType, mode) {
        super();
...
class UnsafePointerType extends Type {
...
exports.UnsafePointerType = UnsafePointerType;
...
var GroupKind;
(function (GroupKind) {
    GroupKind[GroupKind["Free"] = 0] = "Free";
    GroupKind[GroupKind["Bound"] = 1] = "Bound";
})(GroupKind = exports.GroupKind || (exports.GroupKind = {}));
class Group {
    constructor(kind, name) {
        this.counter = Group.groupCounter++;
        this.kind = kind;
        if (name) {
            this.name = name;
        }
...
exports.Taint = Taint;
function combineRestrictions(r1, r2) {
    if (!r1) {
        return r2;
    }
    if (!r2) {
...
exports.combineRestrictions = combineRestrictions;
// Implements restrictions
class RestrictedType extends Type {
    constructor(elementType, r = null) {
        super();
        this.elementType = elementType;
        if (r) {
...
exports.RestrictedType = RestrictedType;
class ArrayType extends Type {
    constructor(elementType, size) {
        super();
        this.elementType = elementType;
        this.size = size;
    }
...
class SliceType extends Type {
    ...
    getElementType() {
        ...
        return this.arrayType.elementType.elementType;
    }
    toString() {
        if (this.name) {
            return this.name;
...
class ArrayLiteralType extends Type {
    constructor(types) {
        super();
        this.types = types;
    }
...
exports.ArrayLiteralType = ArrayLiteralType;
// ObjectLiteralTypes are created while parsing and are then unified.
// They are gone after type checking.
class ObjectLiteralType extends Type {
    constructor(types) {
        super();
        this.types = types;
    }
...
// They are gone after type checking.
class TupleLiteralType extends Type {
    constructor(types) {
        super();
        this.types = types;
    }
...
exports.TupleLiteralType = TupleLiteralType;
...
var GroupCheckFlags;
(function (GroupCheckFlags) {
    GroupCheckFlags[GroupCheckFlags["None"] = 0] = "None";
    GroupCheckFlags[GroupCheckFlags["AllowIsolates"] = 1] = "AllowIsolates";
    GroupCheckFlags[GroupCheckFlags["ForbidIsolates"] = 2] = "ForbidIsolates";
    GroupCheckFlags[GroupCheckFlags["IsolatesMask"] = 3] = "IsolatesMask";
    GroupCheckFlags[GroupCheckFlags["NotIsolateMask"] = 252] = "NotIsolateMask";
    GroupCheckFlags[GroupCheckFlags["NoSideEffects"] = 4] = "NoSideEffects";
    GroupCheckFlags[GroupCheckFlags["AllowUnavailableVariable"] = 8] = "AllowUnavailableVariable";
    GroupCheckFlags[GroupCheckFlags["AllowUnavailableVariableMask"] = 247] = "AllowUnavailableVariableMask";
})(GroupCheckFlags || (GroupCheckFlags = {}));
class TypeChecker {
    constructor(pkg) {
        // List of all interfaces. These are checked for possible errors after they have been defined.
        this.ifaces = [];
        this.structs = [];
        this.templateTypeInstantiations = new Map();
        this.templateFunctionInstantiations = new Map();
        this.typedefs = [];
        this.functions = [];
        this.globalVariables = [];
        this.stringLiteralTypes = new Map();
        if (!TypeChecker.t_bool) {
            ...
            TypeChecker.t_uint = new BasicType("uint");
    ...
    createType(tnode, scope, mode = "default") {
    ...
    createTypeIntern(tnode, scope, mode = "default") {
        ...
        if (tnode.op == "basicType") {
            ...
            if (this.isSafePointer(c)) {
                let ptr = RestrictedType.strip(c);
                ptr.elementType = this.makeConst(ptr.elementType, tnode.loc);
                return c;
            }
            else if (this.isSlice(c)) {
                let ptr = RestrictedType.strip(c);
                ptr.arrayType = this.makeConst(ptr.arrayType, tnode.loc);
                return c;
            }
            // TODO: Map
    ...
    createArrayType(tnode, scope, t, mode) {
        let e = this.createType(tnode.rhs, scope, mode ? mode : "default");
        t.elementType = e;
        return t;
    }
    createOrType(tnode, scope, t, mode) {
    ...
    createInterfaceType(tnode, scope, iface, mode) {
        if (!iface) {
        ...
        for (let mnode of tnode.parameters) {
            if (mnode.op == "extends") {
                ...
                let ft = this.createType(mnode, scope, mode ? mode : "default");
                ...
                let ptrMode = "reference";
                if ((mnode.lhs.flags & ast_1.AstFlags.ReferenceObjectMember) == ast_1.AstFlags.ReferenceObjectMember) {
    ...
    createStructType(tnode, scope, s, mode) {
        if (!s) {
            s = new StructType();
            s.pkg = this.pkg;
            s.loc = tnode.loc;
        ...
        for (let fnode of tnode.parameters) {
            if (fnode.op == "extends") {
                let ext = this.createType(fnode.rhs, scope, mode ? mode : "default");
    ...
    instantiateTemplateType(t, types, loc, mode) {
    ...
    instantiateTemplateMemberFunction(t, s, m) {
        let scope = new Scope(t.parentScope);
     ...
     */
    instantiateTemplateFunctionFromNode(tnode, scope) {
     ...
     */
    instantiateTemplateFunction(t, types, loc) {
        ...
        if (a) {
            for (let f of a) {
                ...
                for (let k = 0; k < types.length; k++) {
                    let type = types[k];
                    let hasType = f.type.templateParameterTypes[k];
                    if (!this.checkTypeEquality(type, hasType, loc, false)) {
                        ok = false;
                        break;
                    }
    ...
    static mangleTemplateParameters(types) {
        let str = "<";
        for (let g of types) {
    ...
    createFunction(fnode, parentScope, registerScope, templateBase = null, templateParameterTypes = null) {
        if (!fnode.name) {
        ...
        if (fnode.lhs) {
            ...
            let mode = "reference";
            if ((fnode.lhs.flags & ast_1.AstFlags.ReferenceObjectMember) == ast_1.AstFlags.ReferenceObjectMember) {
    ...
    createVar(vnode, scope, needType = true, isConst = false, isGlobal = false) {
        let v = new Variable();
        v.loc = vnode.loc;
        v.name = vnode.value;
        v.isGlobal = isGlobal;
    ...
    createImport(inode, scope) {
        if (inode.rhs.op == "importWasm") {
            let ip;
            let importPath = inode.rhs.rhs.value;
            if (!inode.lhs) {
            ...
            let importPath = inode.rhs.value;
            let p = pkg_1.Package.resolve(importPath, inode.rhs.loc);
            let ip;
            if (!inode.lhs) {
    ...
    createFunctionImport(namespace, fnode, scope) {
        let f = new Function();
    ...
    checkVarAssignment(isConst, scope, vnode, rtype, rnode = null) {
    ...
    checkAssignment(scope, vnode, rtype, rnode = null) {
        let rtypeStripped = RestrictedType.strip(rtype);
        if (vnode.op == "tuple") {
            if (!(rtypeStripped instanceof TupleType) && !(rtypeStripped instanceof TupleLiteralType)) {
    ...
    checkStatement(snode, scope, scopeExit) {
        if (snode.op == "comment") {
            return;
        }
        if (!scopeExit.fallthrough) {
        ...
        switch (snode.op) {
            ...
            case "spawn":
                {
                    ...
                    if (snode.rhs.lhs.type.returnType != TypeChecker.t_void) {
            ...
            case "copy":
                ...
                let t = RestrictedType.strip(snode.lhs.type);
                if (this.isConst(t.arrayType)) {
                    throw new TypeError("'copy' requires a non-const slice as its first argument", snode.lhs.loc);
                }
                let e = RestrictedType.strip(t.getElementType());
                let t2 = RestrictedType.strip(snode.rhs.type);
    ...
    checkExpression(enode, scope) {
        switch (enode.op) {
            ...
            case ">=":
                ...
                if ((enode.op == "+" || enode.op == ">" || enode.op == "<" || enode.op == ">=" || enode.op == "<=") && this.isString(enode.lhs.type)) {
                    ...
                    if ((enode.lhs.op == "int" || enode.lhs.op == "float") && (enode.rhs.op == "int" || enode.rhs.op == "float")) {
                        ...
                        let l = parseFloat(enode.lhs.value);
                        let r = parseFloat(enode.rhs.value);
                        switch (enode.op) {
                            case "+":
            ...
            case ".":
                {
                    this.checkExpression(enode.lhs, scope);
                    let type = this.stripType(enode.lhs.type);
            ...
            case ":":
                {
                    ...
                    if (t instanceof ArrayType) {
                        ...
                        enode.type = new SliceType(enode.lhs.type, "local_reference");
                        if (isConst) {
                            enode.type = this.applyConst(enode.type, enode.loc);
                        }
                    }
            ...
            case "[":
                {
                    ...
                    if (t instanceof TupleType) {
                        ...
                        isConst = isConst || this.isConst(t.elementType);
            ...
            case "is":
                {
                    ...
                    if (this.isOrType(enode.lhs.type)) {
                        let ot = this.stripType(enode.lhs.type);
                        let found = false;
                        ...
                        if (!this.isSafePointer(t) || !this.isStruct(RestrictedType.strip(RestrictedType.strip(t).elementType))) {
            ...
            case "typeCast":
                {
                    ...
                    if ((left == TypeChecker.t_float || left == TypeChecker.t_double) && this.isIntNumber(right)) {
                        // Ints can be converted to floats
                        enode.type = t;
                    }
                    else if (this.isIntNumber(left) && (right == TypeChecker.t_float || right == TypeChecker.t_double)) {
                        // Floats can be converted to ints
                        enode.type = t;
                    }
                    else if (left == TypeChecker.t_float && right == TypeChecker.t_double) {
                        // Doubles can be converted to floats
                        enode.type = t;
                    }
                    else if (left == TypeChecker.t_double && right == TypeChecker.t_float) {
                        // Floats can be converted to doubles
                        enode.type = t;
                    }
                    else if (left == TypeChecker.t_rune && this.isUInt32Number(right)) {
                        // Runes can be converted to uint32 
                        enode.type = t;
                    }
                    else if (this.isUInt32Number(left) && right == TypeChecker.t_rune) {
                    ...
                    else if (left instanceof UnsafePointerType && (right instanceof UnsafePointerType || right instanceof PointerType || this.isString(right) || this.isInt32Number(right))) {
                        // Unsafe pointers to anything, safe pointers to anything, strings and 32-bit integers can be converted to any unsafe pointer
                        enode.type = t;
                    }
                    else if ((left == TypeChecker.t_bool || this.isIntNumber(left)) && (right == TypeChecker.t_bool || this.isIntNumber(right)) && t != right) {
            ...
            case "append":
                {
                    ...
                    for (let i = 0; i < enode.parameters.length; i++) {
                        ...
                        if (i == 0) {
                            ...
                            let t = RestrictedType.strip(p.type);
                            if (this.isConst(t.arrayType)) {
                            ...
                            if (expand) {
                                ...
                                let e2 = RestrictedType.strip(p.type).getElementType();
                                if (this.isPureValue(e)) {
                                    // Remove constness in case of pure values.
            ...
            if (node.parameters.length == 0) {
                ...
                for (let i = 1; i < node.parameters.length; i++) {
                    if (!this.checkIsAssignableType(t, node.parameters[i].type, node.loc, "assign", false)) {
                        ...
                        t.types.push(node.parameters[i].type);
                    }
                }
    ...
    unifyLiterals(t, node, scope, loc, doThrow = true, templateParams = null, allowPointerIndirection = true) {
        if (templateParams && t instanceof GenericParameter && templateParams.has(t.name)) {
            t = templateParams.get(t.name);
        }
        if (this.isOrType(t)) {
            let orType = this.stripType(t);
        ...
        switch (node.op) {
            ...
            case "array":
                {
                    if (this.isArray(t)) {
                        let arrayType = this.stripType(t);
                        if (arrayType.size != -1) {
            ...
            case "tuple":
                if (this.isTupleType(t)) {
                    let tupleType = this.stripType(t);
            ...
            case "object":
                if (this.isMap(t)) {
                    ...
                    let structType = this.stripType(t);
    ...
    checkIsAssignableNode(to, from, scope, doThrow = true, templateParams = null) {
    ...
    checkIsAssignableType(to, from, loc, mode, doThrow = true, toRestrictions = null, fromRestrictions = null, templateParams = null) {
    ...
    checkFunctionArguments(ft, args, scope, loc, doThrow = true) {
            ...
            for (let i = 0; i < args.length; i++) {
                ...
                if (pnode.op == "unary...") {
                    ...
                    if (ft.hasEllipsis() && i >= ft.parameters.length - 1) {
                        if (!this.checkIsAssignableNode(ft.lastParameter().type.getElementType(), pnode, scope, doThrow)) {
                            return false;
                        }
                    }
                    else {
    ...
    checkTemplateFunctionArguments(t, args, scope, loc) {
        ...
        let lastParameter = t.node.parameters[t.node.parameters.length - 1];
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (pnode.op == "unary...") {
                ...
                if (ellipsis && i >= t.node.parameters.length - 1) {
                    this.checkIsAssignableNode(this.createType(lastParameter, s, "parameter").getElementType(), pnode, scope, true, result);
                }
                else {
    ...
    checkIsIndexable(node, index, indexCanBeLength = false) {
        let t = this.stripType(node.type);
        if (t instanceof ArrayType) {
    ...
    checkIsAddressable(node, scope, withAmpersand, doThrow = true) {
        switch (node.op) {
            case "id":
    ...
    checkIsPointer(node, doThrow = true) {
        let t = this.stripType(node.type);
        if (t instanceof PointerType || t instanceof UnsafePointerType) {
            return true;
        }
    ...
    checkIsString(node, doThrow = true) {
    ...
    checkIsSignedNumber(node, doThrow = true) {
        let t = this.stripType(node.type);
        if (t == TypeChecker.t_float || t == TypeChecker.t_double || t == TypeChecker.t_int || t == TypeChecker.t_char || t == TypeChecker.t_int8 || t == TypeChecker.t_int16 || t == TypeChecker.t_int32 || t == TypeChecker.t_int64) {
            return true;
        }
        if (doThrow) {
    ...
    checkIsUnsignedNumber(node, doThrow = true) {
        let t = this.stripType(node.type);
        if (t == TypeChecker.t_uint || t == TypeChecker.t_byte || t == TypeChecker.t_uint8 || t == TypeChecker.t_uint16 || t == TypeChecker.t_uint32 || t == TypeChecker.t_uint64) {
            return true;
        }
        if (doThrow) {
    ...
    checkIsNumber(node, doThrow = true) {
    ...
    checkIsIntNumber(node, doThrow = true) {
    ...
    checkIsPlatformIntNumber(node, doThrow = true) {
    ...
    checkIsIntNumberOrUnsafePointer(node, doThrow = true) {
        let t = this.stripType(node.type);
        if (this.isIntNumber(t)) {
            return true;
        }
    ...
    checkIsInterface(node, doThrow = true) {
    ...
    checkFunctionEquality(a, b, loc, allowMoreRestrictions, doThrow = true) {
    ...
    checkTypeEquality(a, b, loc, doThrow = true) {
        if (a == b) {
            return true;
        }
    ...
    isLeftHandSide(node, scope, _allowConstVariable = false) {
        if (node.op == "id") {
    ...
    checkIsMutable(node, scope, doThrow = true) {
    ...
    isPlatformIntNumber(type) {
        type = this.stripType(type);
        return (type == TypeChecker.t_int);
    }
    isIntNumber(type) {
        type = this.stripType(type);
        if (type == TypeChecker.t_int || type == TypeChecker.t_uint || type == TypeChecker.t_byte || type == TypeChecker.t_char || type == TypeChecker.t_int8 || type == TypeChecker.t_int16 || type == TypeChecker.t_int32 || type == TypeChecker.t_int64 || type == TypeChecker.t_uint8 || type == TypeChecker.t_uint16 || type == TypeChecker.t_uint32 || type == TypeChecker.t_uint64) {
            return true;
        }
        return false;
    }
    ...
    static hasStrongOrUniquePointers(t) {
    ...
    static hasReferenceOrStrongPointers(t) {
        t = RestrictedType.strip(t);
    ...
    isPureValue(t) {
        t = this.stripType(t);
        if (t == TypeChecker.t_int || t == TypeChecker.t_uint || t == TypeChecker.t_byte || t == TypeChecker.t_char || t == TypeChecker.t_rune || t == TypeChecker.t_bool || t == TypeChecker.t_float || t == TypeChecker.t_double || t == TypeChecker.t_int8 || t == TypeChecker.t_int16 || t == TypeChecker.t_int32 || t == TypeChecker.t_int64 || t == TypeChecker.t_uint8 || t == TypeChecker.t_uint16 || t == TypeChecker.t_uint32 || t == TypeChecker.t_uint64 || t == TypeChecker.t_null || t == TypeChecker.t_void) {
            return true;
        }
        if (t instanceof TupleType) {
            for (let p of t.types) {
    ...
    applyConst(t, loc) {
        if (this.isSafePointer(t)) {
            ...
            return new PointerType(this.makeConst(t.elementType, loc), t.mode);
            //            if (r) {
            ...
            return new SliceType(this.makeConst(t.arrayType, loc), t.mode);
            //            if (r) {
            //                t =  new RestrictedType(t, r);
            //            }
        }
    ...
    checkGroupsInStatement(snode, scope) {
        switch (snode.op) {
            case "comment":
            case "yield":
                break;
    ...
    checkGroupsInAssignment(snode, scope) {
        if (snode.lhs.op == "id") {
    ...
    checkGroupsInSingleAssignment(ltype, lnode, rightGroup, rnode, rnodeReuse, scope, loc) {
        if (!rnode) {
            throw "Implementation error";
        }
        if (!rightGroup) {
        ...
        let leftGroup = lnode instanceof ast_1.Node ? this.checkGroupsInExpression(lnode, scope, GroupCheckFlags.AllowIsolates | GroupCheckFlags.AllowUnavailableVariable) : lnode;
        let lhsIsVariable = lnode instanceof ast_1.Node ? lnode.op == "id" : false;
        let lhsVariable = null;
        if (lhsIsVariable) {
            lhsVariable = scope.resolveElement(lnode.value);
        }
        ...
        if (TypeChecker.hasStrongOrUniquePointers(ltype)) {
            if (rhsIsVariable) {
                ...
                rnode.flags |= ast_1.AstFlags.ZeroAfterAssignment;
            }
    ...
    checkGroupsInExpression(enode, scope, flags) {
        let origFlags = flags;
        /*
            ...
            case "append":
                {
                    ...
                    for (let i = 0; i < enode.parameters.length; i++) {
                        ...
                        if (i == 0) {
                            group = g;
                            ltype = RestrictedType.strip(p.type).getElementType();
                        }
                        else {
                            if (p.op == "unary...") {
    ...
    checkGroupsInFunctionArguments(ft, defaultGroup, args, scope, loc) {
        let groups = new Map();
        if (defaultGroup) {
        ...
        if (args) {
            for (let i = 0; i < args.length; i++) {
                ...
                if (args[i].op == "unary...") {
                    ...
                    if (ft.hasEllipsis() && i >= ft.parameters.length - 1) {
                        param = ft.lastParameter();
                        ltype = this.arrayElementType(this.sliceArrayType(param.type));
                    }
                    else {
