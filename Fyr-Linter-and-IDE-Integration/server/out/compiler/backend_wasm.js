...
const binarybuffer_1 = require("./binarybuffer");
class Wasm32Storage {
}
exports.Wasm32Storage = Wasm32Storage;
class Wasm32LocalVariableList {
    constructor(localsUsed) {
        this.locals = [];
        this.used = [];
...
class Wasm32Backend {
    constructor() {
        this.copyFunctionIndex = "$copy";
        this.allocFunctionIndex = "$alloc";
        this.hashStringFunctionIndex = "$hashString";
        this.createMapFunctionIndex = "$createMap";
        this.setMapFunctionIndex = "$setMap";
        this.lookupMapFunctionIndex = "$lookupMap";
        this.removeMapKeyFunctionIndex = "$removeMapKey";
        this.setNumericMapFunctionIndex = "$setNumericMap";
        this.lookupNumericMapFunctionIndex = "$lookupNumericMap";
        this.removeNumericMapKeyFunctionIndex = "$removeNumericMapKey";
        this.scheduleCoroutineFunctionIndex = "$scheduleCoroutine";
        this.currentCoroutineFunctionIndex = "$currentCoroutine";
        this.createCoroutineFunctionIndex = "$createCoroutine";
        this.heapSize = 16 << 16; // 1 MB heap
        this.stackSize = 1 << 16; // 64kb Stack    
        this.tr = new ssa_1.SMTransformer();
        this.optimizer = new ssa_1.Optimizer();
        this.stackifier = new ssa_1.Stackifier(this.optimizer);
        this.funcs = [];
        this.globalVarStorage = new Map();
        ...
        this.varsFrameHeader = new ssa_1.StructType();
    ...
    */
    ...
    importFunction(name, from, type) {
        ...
        for (let p of type.params) {
            if (!(p instanceof ssa_1.StructType)) {
                wt.params.push(this.stackTypeOf(p));
            }
        }
        if (type.result) {
            if (!(type.result instanceof ssa_1.StructType)) {
    ...
    generateModule(emitIR, initPackages, duplicateCodePackages) {
        ...
        for (let v of this.globalVariables) {
            if (v.isConstant && typeof (v.constantValue) == "string") {
                ...
                let s = { storageType: "global_strings", offset: offset };
                this.globalVarStorage.set(v, s);
            }
            else if (v.isConstant && v.constantValue instanceof Array) {
                let data = this.encodeLiteral(v.type, v.constantValue);
                let offset = this.module.addBinary(data);
                let s = { storageType: "global_heap", offset: offset };
                this.globalVarStorage.set(v, s);
            }
            else if (v.addressable || v.type instanceof ssa_1.StructType || v.type == "ptr") {
                let offset = this.module.addGlobalStruct(ssa_1.sizeOf(v.type));
                let s = { storageType: "global_heap", offset: offset };
                this.globalVarStorage.set(v, s);
            }
            else {
                let s = { storageType: "global", offset: index };
                this.globalVarStorage.set(v, s);
    ...
    generateFunction(n, f, emitIR) {
        if (n.kind != "define" || (!(n.type instanceof ssa_1.FunctionType))) {
            throw "Implementation error";
        }
        this.steps = [];
        this.stepCode = [];
        this.stepsByName = new Map();
        this.asyncCalls = [];
        this.asyncCallCode = [];
        this.resultFrame = new ssa_1.StructType();
        this.paramsFrame = new ssa_1.StructType();
        this.varsFrame = new ssa_1.StructType();
        this.varStorage = new Map();
        this.varGCStorage = new Map();
        this.varAsyncStorage = new Map();
        this.varBinaryConstants = new Map();
        this.tmpLocalVariables = new Map();
    ...
    generateSyncFunction(n, wf, emitIR) {
        let ircode = "";
        if (n.kind != "define" || (!(n.type instanceof ssa_1.FunctionType))) {
            throw "Implementation error";
        }
        ...
        if (emitIR) {
            ...
            ircode += ssa_1.Node.strainToString("", n) + "\n";
            for (let v of this.varStorage.keys()) {
                let s = this.varStorage.get(v);
    ...
    generateAsyncFunction(n, wf, emitIR) {
        let ircode = "";
        if (n.kind != "define" || (!(n.type instanceof ssa_1.FunctionType))) {
            throw "Implementation error";
        }
    ...
    analyzeVariableStorage(start, end, locals) {
        let n = start;
        for (; n;) {
            ...
            if (n.kind == "decl_result" && (this.wfIsAsync || n.type instanceof ssa_1.StructType)) {
                // Structs are returned via the heap stack.
                // If async, everything is returned via the heap stack.
                let index = this.resultFrame.addField(n.assign.name, n.type);
                let s = { storageType: "result", offset: index };
                ...
                let s = { storageType: "local_result", offset: this.wf.results.length };
                this.wf.results.push(this.stackTypeOf(n.type));
                this.varStorage.set(n.assign, s);
                this.returnVariables.push(n.assign);
                n = n.next[0];
                continue;
            }
            else if (n.kind == "decl_param" && n.type instanceof ssa_1.StructType) {
                // Structs are always passed on the stack
                let index = this.paramsFrame.addField(n.assign.name, n.type);
                let s = { storageType: "params", offset: index };
                this.varStorage.set(n.assign, s);
                ...
                let s = { storageType: "local", offset: this.wf.parameters.length };
                let t = this.stackTypeOf(n.type);
                this.wf.parameters.push(t);
                this.varStorage.set(n.assign, s);
                if (n.assign.gcDiscoverable) {
                    let index = this.varsFrame.addField("$param" + s.offset.toString(), t);
                    let sAlternative = { storageType: "vars", offset: index };
    ...
    assignVariableStorage(v, locals) {
        if (v.name == "$mem") {
            return;
        }
        ...
        if (!v.usedInMultipleSteps && !(v.type instanceof ssa_1.StructType) && !v.gcDiscoverable && !v.addressable) {
            // Non-struct variables (which are not GC-relevant) are stored in local variables.
            let index = locals.allocate(v.type);
            let s = { storageType: "local_var", offset: index };
            this.varStorage.set(v, s);
        }
        else if (v.usedInMultipleSteps && !(v.type instanceof ssa_1.StructType) && !v.addressable) {
            ...
            let sAlternative = { storageType: "vars", offset: indexAlternative };
            this.varAsyncStorage.set(v, sAlternative);
        }
        else if (!(v.type instanceof ssa_1.StructType) && !v.addressable) {
            ...
            let s = { storageType: "vars", offset: index };
            this.varStorage.set(v, s);
        }
     ...
     */
    emitStep(step, start, end, code, depth) {
        code.push(new wasm.End());
        this.emitCode(step, start, end, code, depth, 0);
    }
    emitCode(step, start, end, code, depth, additionalDepth) {
        let n = start;
        for (; n && n != end;) {
            ...
            if (n.kind == "step") {
                ...
                for (let i = 1; i < n.args.length; i++) {
                    if (n.type.params[i - 1] instanceof ssa_1.FunctionType) {
                        throw "Implementation error";
                    }
                    // Pointers as arguments must be passed on the stack
                    if (n.type.params[i - 1] instanceof ssa_1.StructType || n.type.params[i - 1] == "ptr") {
                ...
                code.push(new wasm.GetLocal(this.spLocal));
                // Call the function
                code.push(new wasm.Call(n.args[0]));
                ...
                if (n.assign) {
                    if (n.type.result instanceof ssa_1.StructType) {
                        ...
                        let width = this.stackTypeOf(n.type.result);
                        let asWidth = null;
                ...
                if (n.args[2] instanceof ssa_1.Node && n.args[2].kind == "call_end") {
                    let call_end = n.args[2];
                    let call_type = call_end.type;
                    // Copy from the stack into the destination
                    code.push(new wasm.GetLocal(this.spLocal));
                    this.emitCopy(n.type, call_type.stackFrame.fieldOffset("$result"), n.args[1], code);
                    ...
                    this.emitAssign(n.type, n.args[2], "heap", n.args[1], code);
                }
                n = n.next[0];
            }
                ...
                if (!(n.args[0] instanceof ssa_1.Variable)) {
                    throw "Implementation error";
                }
                ...
                if (n.args.length == 1 && !this.wfIsAsync && !(n.type instanceof ssa_1.StructType)) {
                    if (this.returnVariables.length != 1) {
                        throw "return with one parameter, but function has no return type";
                    }
                    this.emitAssign(n.type, n.args[0], "wasmStack", 0, code);
                }
                else if (n.args.length == 1 && n.args[0] instanceof ssa_1.Node && n.args[0].kind == "call_end") {
                    let call_end = n.args[2];
                    let call_type = call_end.type;
    ...
    /**
     */
    emitAssign(type, n, dest, destOffset, code) {
        if (dest === null && (n instanceof ssa_1.Variable || typeof (n) == "number" || (n instanceof ssa_1.Node && n.kind != "call" && n.kind != "call_indirect" && n.kind != "spawn" && n.kind != "spawn_indirect" && !n.assign))) {
            throw "Implementation error: No assignment";
        }
        if (type instanceof ssa_1.StructType) {
            if (dest == "wasmStack") {
                throw "Implementation error: StructType on wasmStack is not possible";
            }
            // Synchronous function call that returns a StructType?
            if (n instanceof ssa_1.Node && (n.kind == "call" || n.kind == "call_indirect")) {
                if (!(n.type instanceof ssa_1.FunctionType)) {
                ...
                for (let i = 0; i < n.type.params.length; i++) {
                    ...
                    if (n.type.params[i] instanceof ssa_1.StructType) {
                        //                    code.push(new wasm.Comment(">>parameter " + i.toString()));
                        //                        code.push(new wasm.GetLocal(this.spLocal));
                        //                    code.push(new wasm.Comment("<<parameter " + i.toString()));
                        ...
                        if (n.kind == "call_indirect") {
                            paramTypes.push(this.stackTypeOf(n.type.params[i]));
                        }
            ...
            if (n instanceof ssa_1.Node && n.kind == "struct") {
                ...
                if (this.allMemZero(type, n.args) > 8 && (dest === null || !n.assign)) {
                    ...
                    code.push(new wasm.Constant("i32", ssa_1.sizeOf(type)));
                    code.push(new wasm.GetLocal(this.spLocal));
                ...
                for (let i = 0; i < type.fields.length; i++) {
                    ...
                    let t = f[1];
                    let count = f[2];
                    let size = ssa_1.sizeOf(t);
                    let arrOffset = 0;
                    for (let j = 0; j < count; j++, arrOffset += size) {
                        if (dest == "heapStack") {
                            if (n.args[args] === 0 && !hasMemZero && t instanceof ssa_1.StructType) {
            ...
            if (typeof (n) == "number") {
            ...
            else if (n instanceof ssa_1.Variable) {
            ...
            else if (n instanceof ssa_1.Node) {
                if (n.kind == "copy" || n.kind == "load") {
                    ...
                    if (n.kind == "load") {
                        ...
                        this.emitCopy(type, n.args[1], destOffset, code);
                    }
                    else {
        ...
        if (dest == "heapStack" || dest == "heap") {
            let width = this.stackTypeOf(type);
            let asWidth = null;
            switch (type) {
                case "i8":
                case "s8":
    ...
    emitAssignZeroStruct(local, type, destOffset, code) {
        for (let i = 0; i < type.fields.length; i++) {
            let f = type.fields[i];
            let name = f[0];
            let t = f[1];
            ...
            let arrOffset = 0;
            for (let j = 0; j < count; j++, arrOffset += size) {
                if (t instanceof ssa_1.StructType) {
    ...
    emitCopy(type, srcOffset, destOffset, code) {
        let size = ssa_1.sizeOf(type);
        let align = ssa_1.alignmentOf(type);
        switch (size) {
            case 1:
    ...
    emitAddrOfVariable(v, returnOffset, code) {
    ...
    emitWordAssign(type, n, stack, code) {
        if (stack == null && (n instanceof ssa_1.Variable || typeof (n) == "number" || (n.kind != "call" && n.kind != "call_indirect" && n.kind != "spawn" && n.kind != "spawn_indirect" && !n.assign))) {
            throw "Implementation error: No assignment";
        }
        if (n instanceof ssa_1.Node) {
    ...
    emitWordNode(n, stack, code) {
        if (n.kind == "alloc") {
            ...
            this.emitAddrOfVariable(n.args[0], false, code);
            if (n.assign) {
                this.storeVariableFromWasmStack2("addr", n.assign, stack == "wasmStack", code);
            }
            n = n.next[0];
            ...
            if (n.type instanceof ssa_1.StructType || n.type instanceof ssa_1.FunctionType) {
                throw "Implementation error " + n.toString("");
            }
            ...
            code.push(new wasm.Constant(width, n.args[0]));
            if (n.assign) {
            ...
            code.push(new wasm.Load(width, asWidth, n.args[1]));
            if (n.assign) {
                this.storeVariableFromWasmStack2(n.type, n.assign, stack == "wasmStack", code);
            }
            n = n.next[0];
            ...
            let width = this.stackTypeOf(n.type);
            code.push(new wasm.BinaryInstruction(width, n.kind));
            if (n.assign) {
                this.storeVariableFromWasmStack2(n.type, n.assign, stack == "wasmStack", code);
            }
            n = n.next[0];
            ...
            this.emitWordAssign(n.type, n.args[0], "wasmStack", code);
            code.push(new wasm.Extend(ssa_1.isSigned(n.type)));
            if (n.assign) {
                this.storeVariableFromWasmStack2(n.type, n.assign, stack == "wasmStack", code);
            }
            n = n.next[0];
            ...
            this.emitWordAssign(n.type, n.args[0], "wasmStack", code);
            if (n.kind == "trunc32") {
                code.push(new wasm.Trunc(this.stackTypeOf(n.type), "f32", n.type == "s32" || n.type == "s64"));
            }
            else {
                code.push(new wasm.Trunc(this.stackTypeOf(n.type), "f64", n.type == "s32" || n.type == "s64"));
            }
            if (n.assign) {
                this.storeVariableFromWasmStack2(n.type, n.assign, stack == "wasmStack", code);
            }
            ...
            if (n.kind == "convert64_s") {
                code.push(new wasm.Convert(this.stackTypeOf(n.type), "i64", true));
            }
            else if (n.kind == "convert64_u") {
                code.push(new wasm.Convert(this.stackTypeOf(n.type), "i64", false));
            }
            else if (n.kind == "convert32_s") {
                code.push(new wasm.Convert(this.stackTypeOf(n.type), "i32", true));
            }
            else if (n.kind == "convert32_u") {
                code.push(new wasm.Convert(this.stackTypeOf(n.type), "i32", false));
            }
            if (n.assign) {
                this.storeVariableFromWasmStack2(n.type, n.assign, stack == "wasmStack", code);
            }
            ...
            if (n.type.stackFrame.size > 0) {
                ...
                code.push(new wasm.Comment("Create stack frame for " + n.args[0].toString()));
                code.push(new wasm.GetLocal(this.spLocal));
                code.push(new wasm.Constant("i32", n.type.stackFrame.size));
                code.push(new wasm.BinaryInstruction("i32", "sub"));
                code.push(new wasm.SetLocal(this.spLocal));
            ...
            // Call the function
            if (n.args[0] < 0) {
                if (n.args[0] == pkg_1.SystemCalls.heap) {
                    code.push(new wasm.GetGlobal(this.heapGlobalVariableIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.currentMemory) {
                    code.push(new wasm.CurrentMemory());
                }
                else if (n.args[0] == pkg_1.SystemCalls.growMemory) {
                    code.push(new wasm.GrowMemory());
                }
                else if (n.args[0] == pkg_1.SystemCalls.heapTypemap) {
                    code.push(new wasm.GetGlobal(this.typemapGlobalVariableIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.pageSize) {
                    code.push(new wasm.Constant("i32", 1 << 16));
                }
                else if (n.args[0] == pkg_1.SystemCalls.defaultStackSize) {
                    code.push(new wasm.Constant("i32", this.stackSize));
                }
                else if (n.args[0] == pkg_1.SystemCalls.stackPointer) {
                    code.push(new wasm.GetLocal(this.spLocal));
                }
                else if (n.args[0] == pkg_1.SystemCalls.createMap) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.createMapFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.setMap) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.setMapFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.lookupMap) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.lookupMapFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.removeMapKey) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.removeMapKeyFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.hashString) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.hashStringFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.setNumericMap) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.setNumericMapFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.lookupNumericMap) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.lookupNumericMapFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.removeNumericMapKey) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.removeNumericMapKeyFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.continueCoroutine) {
                    code.push(new wasm.CallIndirect("$callbackFn"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.coroutine) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.currentCoroutineFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.scheduleCoroutine) {
                    code.push(new wasm.GetLocal(this.spLocal));
                    code.push(new wasm.Call(this.scheduleCoroutineFunctionIndex));
                }
                else if (n.args[0] == pkg_1.SystemCalls.abs32) {
                    code.push(new wasm.UnaryInstruction("f32", "abs"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.abs64) {
                    code.push(new wasm.UnaryInstruction("f64", "abs"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.sqrt32) {
                    code.push(new wasm.UnaryInstruction("f32", "sqrt"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.sqrt64) {
                    code.push(new wasm.UnaryInstruction("f64", "sqrt"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.trunc32) {
                    code.push(new wasm.UnaryInstruction("f32", "trunc"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.trunc64) {
                    code.push(new wasm.UnaryInstruction("f64", "trunc"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.nearest32) {
                    code.push(new wasm.UnaryInstruction("f32", "nearest"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.nearest64) {
                    code.push(new wasm.UnaryInstruction("f64", "nearest"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.floor32) {
                    code.push(new wasm.UnaryInstruction("f32", "floor"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.floor64) {
                    code.push(new wasm.UnaryInstruction("f64", "floor"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.ceil32) {
                    code.push(new wasm.UnaryInstruction("f32", "ceil"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.ceil64) {
                    code.push(new wasm.UnaryInstruction("f64", "ceil"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.min32) {
                    code.push(new wasm.BinaryInstruction("f32", "min"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.min64) {
                    code.push(new wasm.BinaryInstruction("f64", "min"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.max32) {
                    code.push(new wasm.BinaryInstruction("f32", "max"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.max64) {
                    code.push(new wasm.BinaryInstruction("f64", "max"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.copysign32) {
                    code.push(new wasm.BinaryInstruction("f32", "copysign"));
                }
                else if (n.args[0] == pkg_1.SystemCalls.copysign64) {
    ...
    needsMemZero(type, args) {
        ...
        for (let i = 0; i < type.fields.length; i++) {
            ...
            for (let j = 0; j < count; j++, arrOffset += size) {
                if (t instanceof ssa_1.StructType) {
                    let a = (args == null || args[argNumber] === 0 ? null : args[argNumber].args);
    ...
    storeVariableFromWasmStack1(type, v, code) {
        let s = this.storageOf(v);
        switch (s.storageType) {
            case "vars":
            case "params":
    ...
    storeVariableFromWasmStack2(type, v, tee, code) {
        let width = this.stackTypeOf(type);
        let asWidth = null;
        switch (type) {
            case "i8":
    ...
    stepNumberFromName(name) {
        return this.stepsByName.get(name);
    }
    isBinaryInstruction(kind) {
        switch (kind) {
            case "add":
            case "sub":
            case "mul":
    ...
    stackTypeOf(t) {
        if (t instanceof ssa_1.PointerType) {
            return "i32";
        }
        switch (t) {
            case "i64":
    ...
    encodeLiteral(type, data) {
        let buf = new binarybuffer_1.BinaryBuffer();
        this.encodeLiteralIntern(type, data, 0, buf);
        return buf.data;
    }
    encodeLiteralIntern(type, data, dataOffset, buf) {
        if (type instanceof ssa_1.StructType) {
            let pos = 0;
            ...
            switch (type) {
                case "i8":
                    buf.appendUint8(data[dataOffset++]);
                    break;
                case "s8":
                    buf.appendInt8(data[dataOffset++]);
                    break;
                case "i16":
                    buf.appendUint16(data[dataOffset++]);
                    break;
                case "s16":
                    buf.appendInt16(data[dataOffset++]);
                    break;
                case "i32":
                    buf.appendUint32(data[dataOffset++]);
                    break;
                case "s32":
                    buf.appendInt32(data[dataOffset++]);
                    break;
                case "i64":
                    // TODO: 64 bit
                    buf.appendUint64(data[dataOffset++]);
                    break;
                case "s64":
                    // TODO: 64 bit
                    buf.appendInt64(data[dataOffset++]);
                    break;
                case "f32":
                    buf.appendFloat32(data[dataOffset++]);
                    break;
                case "f64":
                    buf.appendFloat32(data[dataOffset++]);
                    break;
                ...
                case "ptr":
                    if (typeof (data[dataOffset]) == "string") {
                        let [offset, len] = this.module.addString(data[dataOffset++]);
                        buf.appendPointer(offset);
                    }
                    else {
                        buf.appendPointer(data[dataOffset++]);
                    }
                    break;
                default:
                    throw "Implementation error";
...
exports.Wasm32Backend = Wasm32Backend;
